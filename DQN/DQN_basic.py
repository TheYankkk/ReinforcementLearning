...
if __name__=='__main__':
    ...
    for episode_id in range(num_episodes):
        ...
        epsilon=max(#计算当前探索率
            initial_epsilon*(num_exploration_episode-episode_id)/num_exploration_episode,final_epsilon
        )
        for t in range(max_len_episode):
            ...
            if done:#如果游戏结束，则退出本轮循环，进行下一个episode
            ...
            if len(replay_buffer)>=batch_size:
                ...
                y=batch_reward+(gamma*tf.reduce_max(q_value,axis=1))*(1-batch_done)#计算y值
                with tf.GradientTape() as tape:
                    loss=tf.keras.losses.mean_squared_error(
                        #最小化y和Q value的距离
                        y_true=y,
                        y_pred=tf.reduce_sum(model(batch_state)*tf.one_hot(batch_action,
                                                                           depth=2),axis=1)
